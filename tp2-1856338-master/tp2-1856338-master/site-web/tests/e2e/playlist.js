...
describe("Contenu de la page de Playlist", () => {
  before((browser) => {
    browser.url(`${browser.launch_url}/playlist.html?id=1`).execute(
...
function validatePage(browser, index) {
  browser.url(`${browser.launch_url}/playlist.html?id=${index}`).execute(
    () => Object.assign({}, localStorage),
    [],
    (result) => {
      EXPECTED_PLAYLIST = JSON.parse(result.value.playlist).find(
