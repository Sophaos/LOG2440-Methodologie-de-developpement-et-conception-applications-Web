...
export class Library {
  ...
  searchInFields (searchFields, searchValue, exactMatch) {
    return searchFields.some(field => this.includesSubstring(field, searchValue, exactMatch));
  }
  ...
  search (searchInput, searchSources, exactMatch) {
    ...
    const playlists = searchSources.playlists.filter(playlist =>
      this.searchInFields([playlist.name, playlist.description, playlist.thumbnail], searchValue, exactMatch));
    const songs = searchSources.songs.filter(song =>
      this.searchInFields([song.artist, song.name, song.genre], searchValue, exactMatch));
    this.generateLists(playlists, songs);
  }
}
