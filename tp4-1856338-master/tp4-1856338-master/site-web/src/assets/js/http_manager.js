...
export default class HTTPManager {
  ...
  /**
   * TODO -stpham : implémenter la requête vers le serveur
   * Récupère et retourne toutes les chansons du serveur
   * @returns {Promise} Liste des chansons
   */
  async fetchAllSongs () {
    return HTTPInterface.GET(this.songsBaseURL);
  }
  ...
  async fetchAllPlaylists () {
    return HTTPInterface.GET(this.playlistBaseURL);
  }
  ...
  async getPlaylistById (id) {
    try {
      return HTTPInterface.GET(`${this.playlistBaseURL}/${id}`);
    } catch (err) {
      window.alert(err);
    }
  }
