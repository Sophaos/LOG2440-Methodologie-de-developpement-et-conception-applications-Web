...
export default class PlayListEditor {
  ...
  /**
   * TODO -stpham: implémenter l'envoi des requêtes en fonction de si on veut créer ou modifier une playlist existante
  ...
  async createPlaylist (form, playlistId) {
    ...
    // TODO -stpham: Envoyer la bonne requête
    if (playlistId) {
      // TODO -stpham: Modifer la playlist
      await this.HTTPManager.updatePlaylist(newPlaylist);
    } else {
      // TODO -stpham: Créer la playlist
      await this.HTTPManager.addNewPlaylist(newPlaylist);
    }
  }

  /**
   * TODO -stpham : Supprimer une playlist à travers une requête HTTP
