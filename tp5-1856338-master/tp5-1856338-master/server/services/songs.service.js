...
class SongService {
  ...
  /**
   * TODO -stpham: Implémenter la récupération de toutes les chansons
   *
  ...
  async updateSongLike (id) {
    ...
    const newLikedState = !song.liked;
    await this.collection.updateOne({ id }, { $set: { liked: newLikedState } });
    return newLikedState;
  }

  /**
   * TODO -stpham : Implémenter la recherche pour les 3 champs des chansons. Astuce : utilisez l'opérateur '$or' de MongoDB
   *
  ...
  async search (substring, exact) {
    ...
    const artistFilter = { artist: { $regex: `${substring}`, $options: exact ? "" : "i" } };
    const genreFilter = { genre: { $regex: `${substring}`, $options: exact ? "" : "i" } };
    const filter = { $or: [nameFilter, artistFilter, genreFilter] };
