...
export default function Player() {
  ...
  // TODO -stpham: ajouter une action de jouer la prochaine chanson
  const playNextSong = () => dispatch({ type: ACTIONS.NEXT });;

  // TODO -stpham: ajouter une action de jouer la chanson précédante
  const playPreviousSong = () => dispatch({ type: ACTIONS.PREVIOUS });

  // TODO -stpham: ajouter une action de déplacement dans la barre de progrès
  const seek = (newTime) => dispatch({ type: ACTIONS.SEEK, payload: { time : newTime } });

  // TODO -stpham: ajouter une action d'avancement/recul dans la chanson
  const scrubTime = (delta) => dispatch({ type: ACTIONS.SCRUB, payload: { delta : delta } });

  // TODO -stpham: ajouter une action de fermer/ouvrir le son
  const muteToggle = () => dispatch({ type: ACTIONS.MUTE });

  // TODO -stpham: ajouter une action d'activer ou désactiver le mode "shuffle"
    ...
    <>
      ...
      <div id="controls" className="flex-column">
        <section id="buttons-container" className="flex-row">
          {/*TODO -stpham: géré l'événement 'click' */}
          <button
        ...
        <section id="timeline-container" className="flex-row">
          {/*TODO -stpham: afficher le temps en cours de la chanson */}
          <span id="timeline-current">{currentTime}</span>
          {/*TODO -stpham: afficher le progrès de la chanson */}
          <input
            id="timeline"
            type="range"
            max="100"
